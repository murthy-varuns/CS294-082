...
class model():
    ...
    def __init__(self, version, pretrained):
        ...
        self.neurons = [4096, 4096, 256, 10]
        self.history = {'acc':[], 'cap':[]}
    ...
    def estimate_capacity(self):
        assert(self.capacity_estimator)
        self.neurons[0] = torch.sum(self.model.module.classifier[0].weight, dim=1).nonzero().size(0)
        capacity = self.capacity_estimator.estimate(25088, self.neurons)
        self.history['cap'].append(capacity)
        assert(len(self.history['acc']) == len(self.history['cap']))
